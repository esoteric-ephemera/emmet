...
from emmet.core.utils import emmet_zpath, utcnow
...
class LobsterTaskDocument(StructureMetadata):  # type: ignore[call-arg]
    ...
    def from_directory(
        ...
        if file_paths.get("ICOHPLIST"):
            icohp_list = Icohplist(filename=file_paths["ICOHPLIST"])
        if file_paths.get("ICOOPLIST"):
            icoop_list = Icohplist(filename=file_paths["ICOOPLIST"], are_coops=True)
        if file_paths.get("ICOBILIST"):
            icobi_list = Icohplist(filename=file_paths["ICOBILIST"], are_cobis=True)
        ...
        if file_paths.get("SitePotentials"):
            site_potentials = SitePotential(filename=file_paths["SitePotentials"])

        # Read in Gross Populations
        gross_populations = None
        if file_paths.get("GROSSPOP"):
            gross_populations = Grosspop(filename=file_paths["GROSSPOP"])

        # Read in Band overlaps
        band_overlaps = None
        if file_paths.get("bandOverlaps"):
            band_overlaps = Bandoverlaps(filename=file_paths["bandOverlaps"])
        ...
        if save_computational_data_jsons:
            ...
            if file_paths["COHPCAR"].exists() and doc.cohp_data is None:
                cohp_obj = CompleteCohp.from_file(
                    fmt="LOBSTER",
                    structure_file=file_paths["POSCAR"],
                    filename=file_paths["COHPCAR"],
            ...
            if file_paths["COOPCAR"].exists() and doc.coop_data is None:
                coop_obj = CompleteCohp.from_file(
                    fmt="LOBSTER",
                    structure_file=file_paths["POSCAR"],
                    filename=file_paths["COOPCAR"],
            ...
            if file_paths["COBICAR"].exists() and doc.cobi_data is None:
                cobi_obj = CompleteCohp.from_file(
                    fmt="LOBSTER",
                    structure_file=file_paths["POSCAR"],
                    filename=file_paths["COBICAR"],
